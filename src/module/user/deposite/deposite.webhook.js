...
export const razorpayWebhook = async (req, res) => {
  ...
  /* ── ✅ FIX: notes నుండి ఇక coins తీసుకోవట్లేదు — order-create ఇక
     coins/amount ని notes లో పెట్టదు, plan_id మాత్రమే ఆధారం ── */
  const { userId, plan_id } = notes;
  const paymentId  = payment.id;
  const amountPaid = payment.amount / 100; // cents → USD (Razorpay captured actual amount)
  ...
  try {
    ...
    /* ── Plan verify — coins & price ఎప్పుడూ DB నుండే, notes నుండి కాదు ── */
    ...
    /* ── ✅ FIX: captured amount, DB plan price తో మ్యాచ్ అవ్వాలి ── */
    const expectedAmount = Number(plan.price);
    if (Math.abs(amountPaid - expectedAmount) > 0.01) {
      console.error(
        `❌ Webhook amount mismatch — paymentId:${paymentId} paid:${amountPaid} expected:${expectedAmount}`
      );
      await conn.rollback();
      // Razorpay కి 200 పంపాలి — లేదంటే అది webhook ని పదే పదే retry చేస్తుంది
      return res.json({ received: true });
    }
    ...
    /* ── ✅ FIX: coins ఎప్పుడూ plan.coins నుండే, notes.coins నుండి కాదు ── */
    const parsedCoins  = Number(plan.coins);
    ...
    const [txResult] = await conn.query(
      `INSERT INTO coins_transactions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, 'success', ?, ?, ?)`,
      [
        userId, plan_id, parsedCoins, expectedAmount,
    ...
    await conn.query(
      ...
      [
        ...
        plan.matches, expectedAmount, paymentId, plan.validity_days,
      ]
    );
    ...
    const companyClosing = companyOpening + expectedAmount;
    ...
    await conn.query(
      ...
      [
        ...
        userInfo?.fullname || null,
        userInfo?.email    || null,
        plan.name, expectedAmount, companyOpening, companyClosing, paymentId,
      ]
    );
...
const handlePaymentFailed = async (event, res) => {
  ...
  try {
    ...
    /* ── ✅ FIX: coins ఇక notes నుండి కాదు, plan నుండి fetch ── */
    const [[plan]] = await db.query(
      `SELECT coins FROM subscription_plans WHERE id = ? LIMIT 1`,
      [plan_id]
    );
    ...
    await db.query(
      `INSERT INTO coins_transactions
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, 'failed', ?, ?, ?, ?)`,
      [
        userId, plan_id, plan ? Number(plan.coins) : 0, payment.amount / 100,
