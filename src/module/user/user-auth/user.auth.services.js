...
import jwt       from "jsonwebtoken";
import db        from "../../../config/db.js";
import { sendNoreplyMail } from "../../../utils/mailer.js";

/* ══════════════════════════════════════════
   EMAIL TEMPLATES
══════════════════════════════════════════ */
const otpEmailHtml = (otp, title = "Your OTP") => `
  <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:32px;border:1px solid #eee;border-radius:8px">
    ...
    <p style="color:#555">Use the OTP below to continue. Valid for <b>5 minutes</b>.</p>
    ...
    <p style="color:#bbb;font-size:11px">Pick2Win — play responsibly</p>
  </div>      
`;

const welcomeEmailHtml = (fullname) => `
  <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:32px;border:1px solid #eee;border-radius:8px">
    <h2 style="color:#1a1a1a">Welcome to Pick2Win, ${fullname}! 🎉</h2>
    <p style="color:#555">Your account has been created successfully.</p>
    <p style="color:#555">You've received <b>1 free coin</b> to get started!</p>
    <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
    <p style="color:#bbb;font-size:11px">Pick2Win — play responsibly</p>
  </div>
`;
...
export const resendOtpService = async ({ mobile, email, type }) => {
  ...
  if (type === "mobile") {
    ...
    await sendNoreplyMail({
      to:      session.email,
      subject: "Pick2Win — Resend Email OTP",
      html:    otpEmailHtml(newOtp, "Your New OTP"),
    });
